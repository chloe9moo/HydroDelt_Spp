...
#pull out performance of each ind. spp
spp_imp <- as.data.frame(importance(gf.mod, type = "Species")) %>% 
  rownames_to_column("species")
colnames(spp_imp)[2] <- "rel_err"

#add error and save for later
CU_all <- CU_all %>%
  left_join(., spp_imp, by = "species")
...
check_threshold <- function(spp_name, pred_var, ...) {
  
  if(nrow(CU_all %>% filter(species == spp_name)) == 0) { stop(paste("Species name not found.")) }
  spp.cu <- CU_all %>% filter(species == spp_name)
...
check_threshold(spp_name = "Paraleptophlebia", pred_var = "site_x")
